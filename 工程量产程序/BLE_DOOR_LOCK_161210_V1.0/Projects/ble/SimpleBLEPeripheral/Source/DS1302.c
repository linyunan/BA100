...
{
  P0SEL &= ~(BV(1));    // Configure Port 0.1 as GPIO
  P0SEL &= ~(BV(2));    // Configure Port 0.2 as GPIO
  P0SEL &= ~(BV(3));    // Configure Port 0.3 as GPIO
  P0DIR |= BV(1);       // port 0 pins (P0.1) as output
  P0DIR |= BV(2);       // port 0 pins (P0.2) as output
  P0DIR |= BV(3);       // port 0 pins (P0.3) as output
  P0 &= ~(BV(1));       // pin0.1 on port 0 to low
  P0 &= ~(BV(2));       // pin0.2 on port 0 to low
  P0 &= ~(BV(3));       // pin0.3 on port 0 to low
/*  
  WDS1302(0x8E, 0x00);
  WDS1302(0x90, 0x00);
  WDS1302(0x84, 0x00);
  uint8 x=RDS1302(0x81);
  if((x & 0x80)!=0)
   WDS1302(0x80,0);
*/
}
