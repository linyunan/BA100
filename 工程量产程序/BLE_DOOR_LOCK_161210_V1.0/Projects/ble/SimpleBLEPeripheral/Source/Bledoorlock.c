...
#define DOOR_PERIODIC_EVT_PERIOD                   500
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "**Gemini-BA100**";
  ...
  for(uint8 i=0;i<9;i++)
  {
    switch(i)
    {
      ...
      case 4:
      {
        ...
        uiTempDat=RDS1302(0x80|0x01);
        time_char[2]=((uiTempDat&0x7f)>>4)*16+(uiTempDat&0x0f);
        
        if(time_char[2] > time_char[0])
        {
          break;
        }
        else
      ...
      case 8:
      {
        ...
        while( ((temp2+244)>temp1) || ((temp2>temp1)&&(temp2<(temp1+812))) || ((temp2+244)>0x1FF0) )
        {
          temp2 = RandomNumberGenerator();
        }
...
void BleDoorLock_Init( uint8 task_id )
{
  ...
  uint8 scanRspData1[16] = {0}; 
//  uint8 scanRspData3[16] = {0};
  
  
  LL_Encrypt( key, scanRspData, scanRspData1 );
//  LL_EXT_Decrypt(key, scanRspData1, scanRspData3);
...
//        BackLight_ON();
        ...
        //  唤醒一次执行一次一次性密码时间比较
        osal_start_timerEx(bleDoorLock_TaskID, DOOR_ONCEPSWD_EVT,10);				
...
//        GAP_UpdateAdvertisingData( bleDoorLock_TaskID, TRUE, sizeof(attDeviceName), Name_Adv );
...
//        GAP_UpdateAdvertisingData( bleDoorLock_TaskID, FALSE, sizeof(attDeviceName), Name_Adv );
        ...
        TmLed_Close();
//  	DS1302_InitData();												//  临时全局变量初始化
        Ds1302_Init();
        if( global.MY_KEY_CODE  )
        {
        if((global.sys_UpdataStatus == 0x42 ) || global.app_DownChange == 0x01)
          {
//          global.sys_UpdataStatus &= ~(BV(7));
        ...
        if((global.sys_UpdataStatus != 0 && global.sys_UpdataStatus != 0x42 )||global.app_DownChange == 0x02)
          {
            HAL_DISABLE_INTERRUPTS();							//  进入睡眠后处理被更新的内容
            AT24C64_I2C_Write(sys_config.Recode_Addr_Fir, sizeof(SYS_24C64), (uint8*)&sys_24c64);  
            DelayMS(10);
            if(global.App_Down == 1)
            {
              global.App_Down = 0;
              AT24C64_I2C_Write(sys_config.Recode_Addr_Sec, sizeof(SYS_RECODE), &sys_recode);
            }
            if(global.sys_UpdataStatus&0x10)
            {
              AT24C64_I2C_Write(sys_config.Recode_Addr_Sec, sizeof(SYS_RECODE), &sys_recode);
            }
...
//          DelayMS(10);
//          AT24C64_I2C_Write(0x1000, sizeof(SYS_RECODE), &sys_recode);
//        }
        Back_Light_Off();
//        BackLight_OFF();
...
//        if(osal_memcmpn(xing, global.MvInPswd, 2))
        if(global.MvInPswd[0] == 0x0B)
        {
          osal_memcpy(&global.Del_Phone_User[global.Phone_Index][0], &sys_config.Phone_User[global.addPhoneNum][1], 6);
          global.Del_Phone_Index ++;
          sys_config.Phone_User[global.addPhoneNum][0] = 0xFF;
          osal_memcpyz(&sys_config.Phone_User[global.addPhoneNum][1], 0, 7);		// 删除手机用户成功
          osal_snv_write(0x80, sizeof(SYS_CONFIG), &sys_config);
          global.sys_UpdataStatus |= 0x41;
          send_line(0x1B);
        }
 ...
 // 进入手机模式任务
  ...
  {
    static uint8 replyUpdata[16] = {0x02, 0x53, 0x4A,0x00};
    ...
    switch(global.authoryMode)
    {
      case 1:
        ...
        { 
          if(global.sys_UpdataStatus && !global.longButtonFlag )		// 若是第一次设置管理员手机，更新标志位为0xFF ，要上传信息不能再下位机管理员模式情况下
          {
            global.appUpdata_CRC = 0;
 	         global.sys_UpdataStatus |= 0x40;   // 第七位为结束位
            global.appUpdata_CRC = appData_Crc(replyUpdata, global.appUpdata_CRC, 3);

            DoorProfile_SetParameter(DOORPROFILE_CHAR2, 16, replyUpdata);		//     通知要上传用户信息
...
//    static uint8 openDoor[] = {0x1F, 0x4B, 0x4F};
  ...
  //        global.genPswdFlag = 0;							 // 状态位清零
        ...
        osal_memcpyz(global.MvInPswd, 0xFF, 10);                                     // 清空临时 10位虚位密码
          osal_memcpyz(&sys_24c64.Once_Single[i][0], 0xBB, 16);                             // 同时删除一次性密码
...
//    uint32 currenTime = osal_ConvertUTCSecs(&UTCTimeSys);
    ...
    for(uint8 i=0;i<10;i++)
    {
      ...
      if(currenTime >=( sys_24c64.Once_DayLimit[i][0]+86400))
      {
        ...
        osal_memcpyz(&sys_24c64.Once_OneDay[i][0], 0xBB, 7);  			//若超过一天则自动清除一天权限的一次性密码
      }
      if(currenTime >=( sys_24c64.Once_WeekLimit[i][0]+604800))
      {
        sys_24c64.Once_WeekLimit[i][0] = 0x00;
        osal_memcpyz(&sys_24c64.Once_OneWeek[i][0], 0xBB, 7);  		        //若超过一天则自动清除一星期权限的一次性密码
      }
      if(currenTime >=( sys_24c64.Once_MonthLimit[i][0]+2592000))
      {
        sys_24c64.Once_MonthLimit[i][0]= 0x00;
        osal_memcpyz(&sys_24c64.Once_AMonth[i][0], 0xBB, 7);  			//若超过一天则自动清除一个月权限的一次性密码
      }
    }
    osal_stop_timerEx(bleDoorLock_TaskID, DOOR_ONCEPSWD_EVT);
...
//      osal_start_timerEx( bleDoorLock_TaskID, DOOR_ENCRYPT_EVT, 10 ); 
  ...
  if( events & DOOR_INITCONF_EVT )
  {
    ...
    {
      uint8 test[1] = {0x00};
      if(sys_config.Recode_Addr_Fir == 0 && sys_config.Recode_Addr_Sec == 0)  // 未经过配置，第一次复位进入测试代码。
      {
        send_line(0x00);
        Back_Light_On();
        send_line(0x12);
...
//    return ( events ^ DOOR_INITCONF_EVT );
  ...
  {
    uint8 * pt = osal_mem_alloc(8);
    uint8 static test[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
    osal_stop_timerEx(bleDoorLock_TaskID, DOOR_OUTTIME_EVT);
    if( global.menu_Num == 5 )
    {
      if(!osal_memcmpn(&sys_24c64.Tm_User[9][0] ,test, 6) )
      {
        send_line(0x28);  // 卡号已满
        global.menu_Index = 0;
        osal_memcpyz(global.MvInPswd, 0xFF, 10);
    ...
    if(Read1990a(pt)) 
    {
      ...
      if(global.menu_Num == 5)            //  读取TM 卡成功，确认添加请按 "#" 号键
      {
        for(uint8 i=0;i<10;i++)
        {
         if(osal_memcmpn(&sys_24c64.Tm_User[i][0],&pt[1],6))
         {
            send_line(0x33);  // 卡号用户存在
            global.menu_Index = 0;
            osal_memcpyz(global.MvInPswd, 0xFF, 10);
            global.menu_Num = 1;
            osal_start_timerEx(bleDoorLock_TaskID, DOOR_OUTTIME_EVT, 6000);
            osal_stop_timerEx(bleDoorLock_TaskID, DOOR_READCAR_EVT);
            osal_mem_free(pt);
            return ( events ^ DOOR_READCAR_EVT );
         }
...
//      return ( events ^ DOOR_READCAR_EVT );
  ...
  if( events & DOOR_BATTERY_EVT )
  {
    ...
    global.batteryNum ++ ;
    if( ( global.batteryNum > 6 ) && ( global.batteryNum <= 7 ) )
    {
...
//        osal_stop_timerEx(bleDoorLock_TaskID, DOOR_PERIODIC_EVT);
        send_line(0x25);
        osal_start_timerEx(bleDoorLock_TaskID, DOOR_BATTERY_EVT,500);
      }
      else
      {
        global.batteryNum = 0;
...
//  global.addPswdNum = 0;
  ...
  if( keys == HAL_KEY_SW_1 )
  {
    if(Power_OnOff == PowerOn && global.openDoorTurn == 0)
    {
      ...
      if( Button_Num == 0xFF &&  global.batteryNum > 0 )
      {
          osal_set_event(bleDoorLock_TaskID, DOOR_BATTERY_EVT);
      }
      
      if(global.menu_Num >1 && Button_Num != 0xFF)
      {
        osal_stop_timerEx(bleDoorLock_TaskID, DOOR_OUTTIME_EVT);
      }  
      ...
      {  
          if(global.longButtonFlag)  //  在管理员模式下读取按键操作键值
          {
            ...
            case 1:// 进入一级管理   
              ...
              else if( global.menu_Index == 2 &&  Button_Num != 0x0B )
              {
                send_line(0x30);
                global.menu_Index = 0;
                global.menu_Num = 1;
...
//                  GAP_UpdateAdvertisingData( bleDoorLock_TaskID, FALSE, sizeof(attDeviceName), Name_Adv );
                  ...
                  osal_memcpyz(global.MvInPswd, 0xFF, 10);
                  osal_stop_timerEx(bleDoorLock_TaskID, DOOR_ADMIN_PRO_EVT);
//                  LL_Encrypt(key,outPhoneMode,outPhoneMode2);
//                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 3, outPhoneMode2);	    // 通知APP 退出添加模式
            ...
            case 3:                                                             // 修改管理员密码
              if(global.menu_Index == 6 && global.keyRecode_Temp[0] == 0xBB)   // 限定输入的两次密码都为6位数，不带虚位
              {
              ...
              else if( global.menu_Index == 6 && global.keyRecode_Temp[0] != 0xBB)
              {
...
//        global.authoryMode = 0;
        ...
        #endif // (defined HAL_LCD) && (HAL_LCD == TRUE)
        osal_stop_timerEx( bleDoorLock_TaskID, DOOR_SECKEY_EVT ); 
        if (Power_OnOff == PowerOff || (global.AddPhoneFlag == 0 && global.longButtonFlag))
        {
          // 断开所有连接
          GAPRole_TerminateConnection();
    ...
    case GAPROLE_CONNECTED:
      {        
        ...
        if(global.AddPhoneFlag == 1 && global.longButtonFlag == TRUE)        //添加模式下关闭验证
        {
...
//          osal_set_event(bleDoorLock_TaskID, DOOR_ADMIN_PRO_EVT);
        ...
        {
          osal_start_timerEx( bleDoorLock_TaskID, DOOR_SECKEY_EVT, 5000 ); 
        }
        else      // 有权限后关闭验证
        {
          osal_stop_timerEx( bleDoorLock_TaskID, DOOR_SECKEY_EVT);                 //
...
//  static uint8 openDoor[] = {0x1F, 0x4B, 0x4F};
  ...
  uint8 * pt= osal_mem_alloc(8);
  uint8 initial_advertising_enable = FALSE;   
  if((global.authoryMode != 0) && (global.DoorOpenStatus == 1) && (global.sys_UpdataStatus == 0 ))  //|| global.sys_UpdataStatus == 0x40
  {           
    // 断开所有连接
    GAPRole_TerminateConnection();
     // 设置蓝牙广播状态
...
//  uint8 * responseData2 = osal_mem_alloc(DOORPROFILE_CHAR1_LEN);
  ...
  static uint8 replyUpdata[16] = {0};
//  uint8 * replyUpdata = osal_mem_alloc(16);
  static uint8 s_str1[16] = {0x11, 0x22, 0x33};
  static uint8 s_str2[16] = {0x44, 0x55, 0x66};
//  uint8 timeReq[6];
  static uint8 replyUpdata2[16] = {0x02, 0x53, 0x4A,0x00};
...
//                osal_start_timerEx( bleDoorLock_TaskID, DOOR_POWERONOFF_EVT, 15000 ); 
                osal_start_timerEx( bleDoorLock_TaskID, DOOR_POWERONOFF_EVT, 15000 ); 
                if(global.sys_UpdataStatus && !global.longButtonFlag)		// 若是第一次设置管理员手机，更新标志位为0xFF ，要上传信息不能再下位机管理员模式情况下
                {
                  global.appUpdata_CRC = 0;
                  ...
                  global.appUpdata_CRC = appData_Crc(replyUpdata2, global.appUpdata_CRC, 3);
                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 16, replyUpdata2);		//     通知要上传用户信息
                }
              }
              else
              {
...
//                osal_set_event( bleDoorLock_TaskID, DOOR_PHONE_EVT);		//进入普通手机用户
              ...
              if((responseData[7] & 0xF0) == 0x30 && i != 0)                        //  手机用户删除自己本身  管理员手机不可随意删除
              {
...
//                osal_start_timerEx( bleDoorLock_TaskID, DOOR_ENCRYPT_EVT ,100);
	...
	{
          Motor_Backward();
//          osal_start_timerEx(bleDoorLock_TaskID, DOOR_PASSWORD_MANAGE_EVT, 500);
	}
            ...
            case 0x44:                                                          //  TM 卡信息
              if(global.appUpdata_CRC == responseData[3])
              {
                ...
                for(uint8 i=0; i<10; i++)
                {
                  ...
                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 16, &sys_24c64.Tm_User[i][0]);
                }
...
//                AT24C64_I2C_Write(0x1000, sizeof(SYS_RECODE), &sys_recode);
//                osal_set_event(bleDoorLock_TaskID, DOOR_PERIODIC_EVT);
                DoorProfile_SetParameter(DOORPROFILE_CHAR2, 16, s_str1);		//     上传错误信息  
              }
              else
              {
                global.appUpdata_CRC = 0;
      ...
      switch(responseData[0])
      {
        case 0x01 :     // 删除手机用户
        {
          ...
          if(global.authoryMode == 1 && global.appDedata_CRC == responseData[2])       //只有在管理员权限模式下才可以操作此项
          {
            ...
            osal_memcpy(&global.Del_Phone_User[global.Phone_Index][0], &sys_config.Phone_User[responseData[1]][1], 6);
            global.Del_Phone_Index ++;
            osal_memcpyz(&sys_config.Phone_User[responseData[1]][1], 0, 7);
            replyUpdata[0] = 0x03;
...
//          static uint8 replyUpdata[16] = {0x02, 0x53, 0x4A,0x00};
  ...
  //        uint8 * openDoor = osal_mem_alloc(16);
          static uint8 openDoor[16] = {0x1F, 0x4B, 0x4F};
          
          
          if(global.authoryMode != 0)                                   // 首先必须有权限  再选择开门模式
          {
            sys_recode.open_DoorRec[1] = 2;
            sys_recode.open_DoorRec[2] = global.Phone_Index;
            global.Phone_Index = 0;
            if(global.openDoorMode == 1 && global.AppOpenStatus == 1)                                                   //     APP自动模式开门
            {    
              if(responseData[1] <= global.safety_Rssi[1])	
...
//                global.AppOpenStatus = 0;
  //              global.authoryMode = 0;
                ...
                global.AppOpenStatus = 0;
                global.DoorOpenStatus = 1;
                osal_start_timerEx(bleDoorLock_TaskID, DOOR_PERIODIC_EVT,1000);
              }
          //      osal_mem_free(openDoor);
            }
            
            if(global.openDoorMode == 2 && global.AppOpenStatus == 2)                                                   //     APP点亮屏幕开启模式开门
            {
              if(responseData[1] <= global.safety_Rssi[1])
  //            if(global.safety_Rssi[0] <= global.safety_Rssi[1])				//     只有安全距离下才可以开门
              {
...
//         osal_set_event(bleDoorLock_TaskID, DOOR_PERIODIC_EVT);     
       ...
       case 0x0A:
         ...
         DoorProfile_SetParameter(DOORPROFILE_CHAR2, 16, s_str2);		         //     通知APP  门已开
        break; 
