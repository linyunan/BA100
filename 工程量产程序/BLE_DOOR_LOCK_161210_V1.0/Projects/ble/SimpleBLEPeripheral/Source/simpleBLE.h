...
typedef struct
{
  ...
  uint8 General_Pswd[10][16];		// 用户密码
  uint8 Tm_User[10][16];			//  TM 卡信息
  uint8 Admin_Phone_Flag;               // 管理员设置标志位， 0 为未设置手机管理员 ， 1为已经设置手机管理员
  
  uint8 Once_Single[10][16];			// 一次性密码只使用一次
...
typedef struct
{
  uint8 open_DoorRec[3];           //   开门记录 20  超过20覆盖
  uint8 Open_Recode[15][16];		// 开门记录信息
  uint8 Open_Count;
}SYS_RECODE;
...
typedef struct 
{
  ...
  uint8 MY_KEY_CODE;
  uint8 Del_Phone_User[10][6];
  uint8 Del_Phone_Index;
  uint8 Phone_Index;
  uint8 App_Down;
}GLOBAL_VARIABLE; 
