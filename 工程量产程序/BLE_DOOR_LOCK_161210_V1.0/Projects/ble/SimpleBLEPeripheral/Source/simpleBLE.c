...
void Array_Stack(void)  // 对数组进行更新替换
{
  ...
  {
    sys_recode.Open_Recode[i+1][0] = sys_recode.Open_Recode[i+1][0] -1;
...
//  }
  ...
  for(uint8 i=0;i<10; i++)
  {
    ...
    osal_memcpyz(&sys_config.Phone_User[i][1], 0, 15);
  }
...
void Init_Sys24c64(void)
{
  ...
  for(uint8 i=0;i<10;i++)
  {
    osal_memcpyz(&sys_24c64.General_Pswd[i][0], 0xBB, 16);
  }

  for(uint8 i=0;i<10;i++)
  {
    osal_memcpyz(&sys_24c64.Tm_User[i][0], 0xFF, 16);			//  TM 卡信息
  }
  ...
  for(uint8 i=0;i<10;i++)
  {
    osal_memcpyz(&sys_24c64.Once_Single[i][0], 0xBB,16);		// 一次性密码只使用一次
...
void Init_Recode(void)
{
  ...
  {
    osal_memcpyz(&sys_recode.Open_Recode[i][0], 0, 16);		// 开门记录信息
  }
}
